...
is reachable — pz selects native assets through a RID graph, so a musl host resolves `linux-x64` —
but untested here like every other non-linux-x64 platform. `win-arm64` is not shipped by
`DeltaLake.Net` at all.
...
window needs a stable attempt identity. pz's `OutputSpec.Attempt` (`Node`, `Run`, `Ordinal`) supplies
exactly that: a Delta commit could carry it as a commit property, and the next attempt could read it
back and skip what already landed. **This connector has not implemented that**, so `append` remains
at-least-once — see [concepts/delivery-guarantees.md](concepts/delivery-guarantees.md). Note the
promise such an identity would give is within-run only, so it would close the `pz retry` case above
and not a second `pz run`.

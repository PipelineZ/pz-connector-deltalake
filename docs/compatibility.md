...
| `txn` (application transaction id) | **never written** — it is what an effectively-once append would need. pz 0.2.2 supplies no attempt identity to put in it; pz 0.3's `OutputSpec.Attempt` does, within one run, and this connector has not adopted it yet |
...
| `partition_by` | **unreachable through pz 0.2.2** (`PZ0219`); declarable from pz 0.3 via `ColumnPartitionedWrites` (`reference/write.md`) |
